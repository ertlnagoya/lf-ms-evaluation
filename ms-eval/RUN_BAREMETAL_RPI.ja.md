# Raspberry Pi ベアメタル評価（PREEMPT_RT・コア隔離込み）完全手順
...
```
（ベアメタル評価ランナー `run_baremetal.sh` もこれを best-effort で試みる。）
...
- §Threats to Validity の「virtualized host／bare-metal は今後」を、
